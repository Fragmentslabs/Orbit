 ...
 * toggle, mas sem clique explícito. Espelho do useModelConfigPrompt do desktop.
 *
 * Regras:
 ...
 *   perguntar.
 * - Claim global por tipo de dialog: só o dono do claim o libera quando o
 *   dialog fecha.
 */
...
export interface ModelConfigPromptOptions {
  ...
  orchestra?: boolean
  vision?: boolean
  /** Modelos globais configurados (settings-store) */
...
export function useModelConfigPrompt(options: ModelConfigPromptOptions) {
  ...
  // dialog aberto, evitando claim órfão
  useEffect(() => {
