 ...
 * lá. O timeout é curto de propósito: o Git Credential Manager pode abrir uma
 * janela de login, e travar o processo principal esperando por ela seria pior
 * do que falhar e cair no aviso da UI.
 */
async function readGitHubToken(cwd: string): Promise<string | null> {
  return new Promise((resolve) => {
    const child = spawn('git', ['credential', 'fill'], { cwd, env: GIT_ENV })
    let out = ''
    let settled = false
...
export async function createRemoteRepo(
  ...
  const token = await readGitHubToken(repoPath)
  if (!token) {
    return { ok: false, kind: 'noCredential', message: 'Sem credencial do GitHub no git' }
  }
