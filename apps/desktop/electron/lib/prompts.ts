...
- removeBackground spreads from the EDGES, so it only clears background connected to the border. It works on a flat or nearly flat background; it does not separate hair from a busy scene. The result tells you what fraction was cleared: near 0% means it missed the background colour (raise tolerance or pass it explicitly), near 100% means the tolerance ate the subject.
...
- The source is Markdown and the rendering is ours: headings (# ## ###), lists, tables, quotes, **bold**, *italic*, \`code\`, --- for a rule, a line containing only \\pagebreak for a page break, and <br> for a blank line inside a paragraph (useful for answer space on a form or a test).\n- Emphasis is ONLY with asterisks. Underscores are left alone on purpose, so a fill-in line ("Nome: ______") survives intact.\n- No other HTML is interpreted: <b>, <div>, <table> and the like appear literally in the document. Everything you want formatted goes through the Markdown above.
