...
import { CheckIcon, ChevronUpIcon, CopyIcon, FolderGit2Icon, FolderOpenIcon, FolderTreeIcon, HistoryIcon, FolderIcon } from "lucide-react"
...
type ReaddirResult = { ok: true; entries: DirEntryInfo[] } | { ok: false; error: string }
type ReadFileResult = { content: string } | { error: string }
type GitLogResult = { ok: true; commits: CommitEntry[] } | { ok: false; error: string }

type ViewedFile =
  | { kind: "live"; path: string }
  | { kind: "commit"; repoPath: string; hash: string; path: string; deleted: boolean }
  ...
  return entries.map(entry =>
    entry.isDirectory ? (
      <FileTreeFolder key={entry.path} name={entry.name} path={entry.path}>
        {expandedPaths.has(entry.path) && renderEntries(dirCache[entry.path], dirCache, expandedPaths)}
      </FileTreeFolder>
    ) : (
...
function FolderQuickSwitch({ folders, onFoldersChange }: { folders: string[]; onFoldersChange: (f: string[]) => void }) {
  ...
  const primaryName = folders.length === 0 ? "Sem pasta" : getBaseName(folders[0])
  ...
  return (
    <div className="relative shrink-0 border-t border-sidebar-border p-2 pt-2.5" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
      ...
      >
        ...
        <ChevronUpIcon className={cn("size-3 shrink-0 transition-transform", open && "rotate-180")} />
      </button>
      {open && (
        <div className="absolute bottom-full left-2 right-2 z-50 mb-1 overflow-hidden rounded-lg border bg-popover/70 p-1 text-popover-foreground shadow-md ring-1 ring-foreground/10 backdrop-blur-2xl backdrop-saturate-150">
          ...
          {folders.map(f => (
            ...
            >
              ...
              <span className="flex-1 truncate text-left">{getBaseName(f)}</span>
...
function CodeView({ content, highlighted }: { content: string; highlighted: HighlightedToken[][] | null }) {
    ...
    <div className="flex min-w-0 font-mono text-xs">
      ...
      <div className="min-w-0 flex-1 px-4 py-4">
        {highlighted ? (
          ...
          lines.map((line, i) => (
            <div key={i} className="min-w-0 whitespace-pre leading-5">{line || " "}</div>
          ))
        )}
      </div>
    </div>
...
export function FoldersTab() {
  ...
  const [expandedPaths, setExpandedPaths] = useState<Set<string>>(() => new Set(folders))
  ...
  const [highlighted, setHighlighted] = useState<HighlightedToken[][] | null>(null)
  ...
  const loadDir = useCallback(async (dirPath: string) => {
    ...
    const result = (await window.ipcRenderer.invoke("fs:readdir", dirPath)) as ReaddirResult
    loadingRef.current.delete(dirPath)
    if (result.ok) {
      for (const e of result.entries) entryIsDirRef.current.set(e.path, e.isDirectory)
  ...
  const openCommitFile = useCallback(async (repoPath: string, hash: string, path: string, deleted: boolean) => {
    setViewedFile({ kind: "commit", repoPath, hash, path, deleted })
    setFileLoading(true)
    setFileError(null)
    setFileContent(null)
    const result = (await window.ipcRenderer.invoke("git:showFile", repoPath, hash, path, deleted)) as ReadFileResult
    setFileLoading(false)
    if ("content" in result) setFileContent(result.content)
    else setFileError(result.error)
  }, [])
  ...
  useEffect(() => {
    ...
    let cancelled = false
    highlightLines(fileContent, viewedFile.path, isDark ? "dark" : "light").then(result => {
  ...
  const handleReveal = useCallback(() => {
    ...
    window.ipcRenderer.invoke("shell:showItemInFolder", viewedFile.path).catch(console.error)
  }, [viewedFile])
  ...
  if (folders.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6 text-center">
        <p className="text-sm text-muted-foreground">Nenhuma pasta associada a esta conversa.</p>
  ...
  return (
    <PanelGroup className="min-h-0 min-w-0 flex-1" direction="horizontal">
      <Panel className="min-w-0" defaultSize={60} id="code-viewer" minSize={25} order={1}>
              ...
              <ArtifactHeader className="min-w-0 bg-code-viewer">
                <div className="min-w-0">
                  <ArtifactTitle className="flex items-center gap-1.5 truncate">
                    {viewedFile.kind === "commit" && <HistoryIcon className="size-3.5 shrink-0 text-muted-foreground" />}
                    {getBaseName(viewedFile.path)}
                  </ArtifactTitle>
                  <ArtifactDescription className="truncate">
                    {viewedFile.kind === "commit"
                      ? `${viewedFile.hash.slice(0, 7)} · ${viewedFile.path}`
                      : viewedFile.path}
                  </ArtifactDescription>
                </div>
                <ArtifactActions>
      ...
      >
        ...
        >
          ...
          <TabsContent className="min-h-0 min-w-0 flex-1 overflow-hidden" value="files">
            <ScrollArea className="h-full">
              <FileTree
                ...
                selectedPath={viewedFile?.kind === "live" ? viewedFile.path : undefined}
              >
                {folders.map(folderPath => (
                  <FileTreeFolder key={folderPath} name={getBaseName(folderPath)} path={folderPath}>
                    {expandedPaths.has(folderPath) && renderEntries(dirCache[folderPath], dirCache, expandedPaths)}
                  </FileTreeFolder>
                ))}
              </FileTree>
            </ScrollArea>
          </TabsContent>
          <TabsContent className="min-h-0 min-w-0 flex-1 overflow-hidden" value="commits">
            <ScrollArea className="h-full">
              <div className="flex flex-col gap-2 p-2 text-xs">
                {commitsLoading && (
                  <div className="p-4 text-center text-muted-foreground">Carregando commits...</div>
                )}
                ...
                {!commitsLoading && !commitsError && commits?.length === 0 && (
                  <div className="p-4 text-center text-muted-foreground">Nenhum commit encontrado.</div>
                )}
                {commits?.map(commit => (
                    ...
                    <CommitHeader className="p-2">
                      <CommitInfo className="min-w-0 gap-1">
                        <HoverCard>
                          ...
                          />
                          <HoverCardContent align="start" className="w-64 space-y-1.5" side="right">
                            <p className="font-medium leading-snug break-words">{commit.message}</p>
                            {commit.body && (
                            ...
                            <div className="flex items-center gap-1.5 pt-1 text-[10px] text-muted-foreground">
                              <CommitHash className="shrink-0 text-[10px]">{commit.hash.slice(0, 7)}</CommitHash>
                              <CommitSeparator className="shrink-0" />
                              <span className="truncate">{commit.author}</span>
                              <CommitSeparator className="shrink-0" />
                              <CommitTimestamp className="shrink-0 text-[10px]" date={new Date(commit.date)} />
                            </div>
                          </HoverCardContent>
                        </HoverCard>
                        ...
                        <CommitFiles>
                          {commit.files.map(f => (
                            <CommitFile
                              ...
                              onClick={() => openCommitFile(folders[0], commit.hash, f.path, f.status === "deleted")}
                            >
                              <CommitFileInfo>
